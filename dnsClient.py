...
def dns_query(type, name, server):
    ...
    ARCOUNT = 0
        ...
        # The DNS header is a fixed-size (12 bytes), containing several fields with information about the query.
        # These fields are packed into a binary string in network byte order
        # The values of these fields are combined using bitwise operations (<< for left shift and | for bitwise OR) to form a single 16-bit value,
        # which is then packed into the binary string.
